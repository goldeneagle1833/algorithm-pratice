    ...
    for (let i = 1; i < 100000; i++) {
      ...
      function checkfactors(num) {
        ...
        for (
          ...
          // 9 <= 10 true and
          curFactor += inc // 2 += 1 so the next loop curFactor will start at 3? // 2nd loop  curFactor is 3? i think // 3 += 1 so the next loop is 4?
        ) {
...
function largestCollatz() {
  for (let i = 1000000; i > 0; i--) {
    ...
    function collatz(num) {
      ...
      if (num % 2 === 0) {
        evennum = num / 2;
        collatzSeqence.push(evennum);
      } else {
        oddnum = 3 * num + 1;
        collatzSeqence.push(oddnum);
      }

      if (collatzSeqence[collatzSeqence.length - 1] !== 1) {
        collatz(collatzSeqence[collatzSeqence.length - 1]);
      }
    }
    collatz(i);
...
function checkfactors(num) {
  ...
  for (
    ...
    // 4 <= 6 true
    curFactor += inc // 2 += 1 so the next loop curFactor will start at 3
  ) {
    // the factoring of the num
...
function digSums(pow) {
  ...
  for (let i = 0; i < digString.length; i++) {
    ...
    //console.log(digArrParseArr);
  }
  let digreduce = digArrParseArr.reduce(reducer);
